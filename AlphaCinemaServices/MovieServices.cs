...
namespace AlphaCinemaServices
{
    public class MovieServices : IMovieServices
    {
        ...
        public void AddNewMovie(string name, string description, int releaseYear, int duration)
        {
            ...
            if (movie != null)
            {
                if (movie.IsDeleted)
                {
                    ...
                    var movieProjections = this.unitOfWork.Projections.AllAndDeleted()
                        .Where(p => p.MovieId == movie.Id)
                        .ToList();
                    if (movieProjections != null)
                    {
                        movieProjections.ForEach(movieProjection => movieProjection.IsDeleted = false);
                    }
                    var movieGenres = this.unitOfWork.MovieGenres.AllAndDeleted()
                        .Where(mg => mg.MovieId == movie.Id)
                        .ToList();
                    if (movieGenres != null)
                    {
                        movieGenres.ForEach(movieProjection => movieProjection.IsDeleted = false);
                    }
        ...
        public void DeleteMovie(string movieName)
        {
            ...
            this.unitOfWork.Projections.All()
                .Where(p => p.MovieId == movie.Id)
                .ToList()
                .ForEach(movieProjection => this.unitOfWork.Projections.Delete(movieProjection));
            this.unitOfWork.MovieGenres.All()
                .Where(mg => mg.MovieId == movie.Id)
                .ToList()
                .ForEach(movieGenre => this.unitOfWork.MovieGenres.Delete(movieGenre));
