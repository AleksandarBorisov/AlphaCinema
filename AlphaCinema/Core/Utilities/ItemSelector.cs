    ...
    public class ItemSelector : IItemSelector
    {
        public string DisplayItems(List<string> selection)//, IAlphaConsole alphaConsole
        {
            ...
            {//Последните два елемента са ни координатите
                PrintAtPosition(i == 0 ? selection[i].ToUpper() : selection[i], i * startingRow + offSetFromTop, i == currentIndex ? true : false);
            ...
            while (key.Key != ConsoleKey.Enter)
            {
                ...
                {
                    PrintAtPosition(selection[currentIndex], currentIndex * startingRow + offSetFromTop, false);
                    PrintAtPosition(selection[--currentIndex], currentIndex * startingRow + offSetFromTop, true);
                }
