	...
	public class AddMovieGenre : ICommand
	{
		...
		public void Execute(List<string> parameters)
		{
			...
			try
			{
				...
				Validations(input);
				var movie = movieServices.GetMovieByName(input[0].TrimEnd());
				var genre = genreServices.GetGenreByName(input[1].Trim());

				if (movie == null || genre == null)
				{
					throw new EntityDoesntExistException("\nNo such thing in database");
				}

				movieGenreServices.AddNew(movie, genre);
